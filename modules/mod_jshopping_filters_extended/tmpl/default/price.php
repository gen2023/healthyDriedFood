...
<div class="fblock show_price_detail<?php echo $span;?>" group="price">
	<?php if ($show_dropdown_list) {?>
		<div class="head"><?php print Text::_('PRICE')?></div>
	<?php } ?>
			...
			<div class="filter-price-block">
				<span class="filter_price">		
					<span class="box_price_from">
						<span><?php print Text::_('FROM')?></span>
						<input type = "text" class = "inputbox" name = "fprice_from" id="price_from" placeholder="<?php print Text::_('FROM')?>" size="7" value="<?php if ($fprice_from>0) print $fprice_from?>" />
					</span>
					<span class="box_price_to">
						<span><?php print Text::_('TO')?></span>
						<input type = "text" class = "inputbox" name = "fprice_to"  id="price_to" placeholder="<?php print Text::_('TO')?>" size="7" value="<?php if ($fprice_to>0) print $fprice_to?>" />
					</span>
					...
					<?php if ($show_price_button) {?>
						<span class="box_price_btn">						
							<button class="btn btn-success btn-go" onclick="jshop_filters_submit(<?php print $filter_number?>);return false;"><?php print Text::_('GO')?></button>
						</span>
					<?php }?>
				</span>
			</div>
	...
	<?php if ($show_products_with_old_prices_enabled){?>
		<div class="show_prices">
			<label>
				...
				<?php print Text::_('PRODUCTS_WITH_OLD_PRICES')?>
			</label>
		</div>
	<?php } ?>
	

	<div style="display:none" group_name_price_from="1"><?php print Text::_('JSHOP_SEARCH_PRICE_FROM')?></div>
	<div style="display:none" group_name_price_to="1"><?php print Text::_('JSHOP_SEARCH_PRICE_TO')?></div>
	</div>
