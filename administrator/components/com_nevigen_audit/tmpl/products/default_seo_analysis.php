...
$linkList = 'index.php?option=com_nevigen_audit&view=products';
?>
					...
					<?php foreach ($data as $lang => $count) : ?>
						<?php if ($count > 0) : ?>
							<td class="text-center">
								<div class="dropdown">
									<button class="btn bg-danger dropdown-toggle text-white fs-4"
											type="button"
									...
									<ul class="dropdown-menu">
										<li>
											<a class="dropdown-item"
											   target="_blank"
											   href="<?php echo $linkList; ?>&filter[search]=empty:<?php echo $name . '_' . $lang; ?>">
												<span class="icon-eye"></span>
												<?php echo Text::_('COM_NEVIGEN_AUDIT_VIEWED'); ?>
											</a>
										</li>
										<?php if (PluginHelper::isEnabled('system', 'nevigen_jshop_basic_seo')): ?>
											<li>
												<button class="dropdown-item text-success"
														onclick="window.NevigenJShopBasicSeoConfigurationList.getModalConfiguration('products')">
													<span class="icon-cog"></span>
													<?php echo Text::_('COM_NEVIGEN_AUDIT_REPAIR'); ?>
												</button>
											</li>
										<?php else: ?>
											<li>
												<a href="https://nevigen.com/seo-joomshopping/joomshopping-basic-seo.html"
