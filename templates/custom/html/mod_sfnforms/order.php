...
<div id="mod-sfnforms-container-<?= $uniqid; ?>" class="sfnforms">
	<form id="mod-sfnforms-contact-form-<?= $uniqid; ?>" name="mod-sfnforms-contact-form-<?= $uniqid; ?>" enctype="multipart/form-data" method="post"
		action="<?php echo modSFNFormsHelper::cleanInput(Uri::current()); ?>" class="<?= $class_form ?>">

		<?php echo HTMLHelper::_('form.token'); ?>
		<input type="hidden" name="mod_sfnforms_module_id" value="<?php echo $module->id; ?>" />
		<input type="hidden" name="mod_sfnforms_module_name" value="<?php echo modSFNFormsHelper::cleanInput($module->title); ?>" />
			...
			<div class="control-group">
				<div class="controls">
					<label for="mod-sfnforms-display-consent-<?= $uniqid; ?>" class="checkbox">
						<input type="checkbox" name="mod_sfnforms_display_consent" id="mod-sfnforms-display-consent-<?= $uniqid; ?>" value="1">
						<?= $consent_text; ?>
					</label>
				</div>
			</div>
		...
		foreach ($array_fields as $key => $value) {
			if ($value->type_input == 'hidden') {
				...
				?>
				<input type="hidden" class="ignore <?= $class ?>" name="mod_sfnforms_<?= $value->field_name; ?>-<?= $uniqid; ?>" id="mod-sfnforms-<?= $value->field_name; ?>-<?= $uniqid; ?>"
					value="<?= $field_value ?>" />
			<?php }
		}
		...
		<div class="controls">
			<button type="submit" id="mod-sfnforms-submit-btn-<?= $uniqid; ?>" name="mod_sfnforms_submit-btn-<?= $uniqid; ?>" class="btn btn-primary submit">
