...
<div class="wishlist-module">
        ...
        <?php foreach($wishlist->products as $number_in_cart => $product){ ?>
            <li class="module-item" ca-product_id="<?php print $product['product_id']?>" ca-number="<?php print $number_in_cart?>">
                ...
                <?php if ($params->get("showEan", 1)) : ?>
                    <div class="module-item-id"><span><?php echo Text::_('JSHOP_EAN') ?>:</span> <n><?php print $product['ean']?></n></div>
                <?php endif; ?>
                <div class="module-item-price"><span><?php echo Text::_('JSHOP_PRICE') ?>:</span> <n><?php print Helper::formatprice($product['price'])?></n></div>
    ...
    <div class="no-products-msg" ca-prodcount="<?php print count($wishlist->products)?>">
        <?php print Text::_('JSHOP_WISHLIST_EMPTY')?>
    </div>    
    <div class="module-bottom" ca-prodcount="<?php print count($wishlist->products)?>">
		<div class="module-total">
			<n class="name"><?php print Text::_('JSHOP_TOTAL')?>:</n>
			<span class="val"><?php print Helper::formatprice($wishlist->price_product)?></span>
		</div>
		<div class="module-count">
			<n class="name"><?php print Text::_('JSHOP_PRODUCTS')?>:</n>
			<span class="val"><?php print isset($wishlist->count_product) ? $wishlist->count_product : 0;?></span>			
		</div>		
        <div class="module-checkout">
            <?php /*<a class="module-checkout" href="<?php print $cart->cartAjaxHrefLink->link; ?>">
                <?php print $cart->cartAjaxHrefLink->label; ?>
            </a> */?>
            <a class="module-checkout" href = "<?php print Helper::SEFLink('index.php?option=com_jshopping&controller=wishlist&task=view', 1)?>"><?php print Text::_('GO_TO_WISHLIST')?></a>
        </div>
    </div>
    ...
    <div class="module-item-template" style="display:none">
        <li class="module-item" ca-product_id="" ca-number="">
            ...
            <?php if ($params->get("showEan", 1)) : ?>
                <div class="module-item-code"><span><?php echo Text::_('JSHOP_EAN') ?>:</span> <n></n></div>
            <?php endif; ?>
